...
class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacancyByIdAndFoundThenGetPageWithVacancyAndCities() {
    ...
    @Test
    public void whenRequestVacancyByIdAndNotFoundThenGetErrorPage() {
        ...
        assertThat(model.getAttribute("message"))
                .isEqualTo("Вакансия с указанным идентификатором не найдена");
    }

    @Test
    public void whenPostVacancyUpdateWithFileThenSameDataAndRedirectToVacanciesPage() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenPostVacancyUpdateAndNotFoundThenGetErrorPage() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc", now(), true, 1, 1);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
